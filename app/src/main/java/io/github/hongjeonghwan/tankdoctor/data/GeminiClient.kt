...
object GeminiClient {
    ...
    private val SYSTEM_PROMPT = """
        ...
        - 마릿수는 "현재 물고기"에 적힌 수가 정답입니다. 사진에서 센 수로 마릿수를 바꿔 말하지 마세요.
          사진에는 수초나 유목에 가려 안 보이는 개체가 있고, 같은 개체가 여러 번 보이기도 합니다.
          사진에 보이는 수가 등록된 수와 다르면 "사진에는 N마리만 보여요" 정도로만 언급하고,
          마릿수가 바뀌었다면 기록을 업데이트해 달라고 권하세요. 진단과 계산은 등록된 수로 하세요.
        - "사육 밀집도(앱 계산)"는 등록된 마릿수로 계산한 값이니 그대로 신뢰하고 판단에 쓰세요.
